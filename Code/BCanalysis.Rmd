...
pp_ARG <- pp_ARG + geom_boxplot(width=0.5, aes(fill=Strata), colour="black", outlier.size = 0.3) + 
  ylim(0,0.8) +
...
pp_COL <- pp_COL + geom_boxplot(width=0.5, aes(fill=Strata), colour="black", outlier.size = 0.3) + 
  ylim(0,0.8) + 
  scale_color_manual(values = strataColor, aesthetics = 'fill') + 
...
pp_ECU <- pp_ECU + geom_boxplot(width=0.5, aes(fill=Strata), colour="black", outlier.size = 0.3) + 
  ylim(0,0.8) + 
  scale_color_manual(values = strataColor, aesthetics = 'fill') + 
...
pp_USA <- pp_USA + geom_boxplot(width=0.5, aes(fill=Strata), colour="black", outlier.size = 0.3) + 
  ylim(0,0.8) + 
  scale_color_manual(values = strataColor, aesthetics = 'fill') + 
  labs(title="USA", x="", y=" ") +
  facet_grid(~Site) + 
...
CM_compare_FC <- bind_rows(CM_Visual_FC_long, CM_Robot_FC_long)
...
Let's apply a nested model to test if there is difference between the cover estimates from human and robot for each label
