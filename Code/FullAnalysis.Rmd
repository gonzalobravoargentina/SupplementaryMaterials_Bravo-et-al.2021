...
dfSummary <- dfHuman %>% group_by(Label) %>% 
  summarise(nPoints = n(), Countries = paste0(unique(country), collapse = ", ")) %>% 
  mutate(Percent = round(100*nPoints/nrow(dfHuman),1)) %>% 
  arrange(-nPoints)
...
CM_compare_FC$countryCode <- str_sub(CM_compare_FC$Name,1,2)
CM_compare_FC$siteCode <- ifelse(CM_compare_FC$countryCode=='US', str_sub(CM_compare_FC$Name,3,6),str_split(CM_compare_FC$Name, "_", simplify = T)[,3] )
CM_compare_FC$strataCode <- ifelse(CM_compare_FC$countryCode=='US', str_split(CM_compare_FC$Name, "_", simplify = T)[,3],
                            str_split(CM_compare_FC$Name, "_", simplify = T)[,4])
CM_compare_FC$strataCode[CM_compare_FC$strataCode=='high'] <- "HT"
CM_compare_FC$strataCode[CM_compare_FC$strataCode=='low'] <- "LT"
...
CM_compare$Cover[is.na(CM_compare$Cover)] <- 0
